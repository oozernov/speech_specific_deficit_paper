...
tone=merge(jnd_wide,mean_thresh)
tone=merge(tone,d_ntd)
...
adaptor_diff <- d %>%
  group_by(adaptor, PartID,  step) %>%
  dplyr::summarize(mean_b = mean(b, na.rm = TRUE)) %>%
  group_by(PartID) %>%
  spread(adaptor, mean_b) %>%
  ungroup() %>%
  mutate(diff = d - b) %>%
  dplyr::select(PartID, step, diff)

adaptor_diff <- adaptor_diff %>%
  group_by(PartID) %>%
...
select_adap<-merge(select_long_wide,adaptor_diff,'PartID')
...
write.csv(tone_sa,'ind_tone_selct_102020.csv')
...
#### Plot distribution ####
ggplot(d_glm_fit, aes(x = slope_coef)) +
  geom_density(alpha = .5) +
  labs(title = "Transformed Slope Distribution") +
  theme(plot.title = element_text(hjust = 0.5))
...
adaptor_diff2 <- adaptor_diff %>%
  group_by(PartID) %>%
  dplyr::summarize(mean_diff = mean(diff, na.rm = TRUE)) %>%
  #left_join(d_conds, by = "PartID") %>%
  #unique()

hist(adaptor_diff2$mean_diff)
#this one is old 10/20/20
#adaptor_diff <- d_glm_fit %>%
 # dplyr::select(PartID, adaptor, inflection_step) %>%
  #spread(adaptor, inflection_step) %>%
  #plyr::mutate(diff = d - b)
...
select_long<-d_glm_fit%>%dplyr::select(PartID,adaptor,slope_coef)
select_wide<-spread(select_long,adaptor,slope_coef)
select_adap<-merge(select_wide,adaptor_diff2,'PartID',all = TRUE)
step8<-step8%>%dplyr::select(PartID, diff_8)
select_adap<-merge(select_adap,step8,'PartID')

tone_sa=merge(tone,select_adap,"PartID")
write.csv(tone_sa,'ind_tone_select_child_122020.csv')
